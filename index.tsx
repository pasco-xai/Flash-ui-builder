...
function App() {
  ...
  const [currentSessionIndex, setCurrentSessionIndex] = useState<number>(() => {
      try {
          const saved = localStorage.getItem('flash_ui_index');
  ...
  useEffect(() => {
      try {
          ...
          localStorage.setItem('flash_ui_index', currentSessionIndex.toString());
      } catch (e) {
...
Return ONLY RAW HTML. No markdown fences.
  ...
  const hasStarted = currentSessionIndex !== -1 || isLoading;
  const currentSession = sessions[currentSessionIndex];
  ...
  return (
    <>
            ...
            {drawerState.mode === 'history' && (
                <ul className="history-list">
                    {sessions.map((sess, idx) => (
                        <li key={sess.id} className={`history-item ${idx === currentSessionIndex ? 'active' : ''}`} onClick={() => jumpToSession(idx)}>
                             <div className="history-item-content">
                                 <div className="history-item-prompt">{sess.prompt}</div>
                                 <div className="history-item-time">{new Date(sess.timestamp).toLocaleString()}</div>
                             </div>
                             <button className="history-item-delete" onClick={(e) => handleDeleteSession(e, idx)}>
                                 <TrashIcon />
                             </button>
                        </li>
                    ))}
                    {sessions.length === 0 && <div className="loading-state">No history yet</div>}
                </ul>
            )}
        ...
        <div className="top-left-controls">
            <button className="icon-button" onClick={handleClearUI} title="New Session">
                <PlusIcon />
            </button>
            <button className="icon-button" onClick={handleShowHistory} title="View History">
                <HistoryIcon />
            </button>
        </div>
